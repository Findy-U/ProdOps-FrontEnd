...
def df_ind(df):
    ...
    df['time_to_complete'] = round(
        (df['closed_at'] - df['created_at']).dt.days, 2)
...
def df_ind_pessoal():
    ...
    df_ind_pessoal = pd.DataFrame(
        columns=['assignee', 'numero_tarefas', 'tempo_trabalho'])
...
def plot_histogram_blocked_vs_unblocked(df):
    ...
    fig = go.Figure(
        data=[go.Bar(x=['Blocked', 'Unblocked'], y=[blocked, unblocked])])
    fig.update_layout(
...
def plot_line_chart_by_month(df, indicator):
    ...
    fig = go.Figure(data=[go.Scatter(x=indicator_sum_by_month.index,
                    y=indicator_sum_by_month.values, mode='markers+lines')])
    fig.update_layout(
